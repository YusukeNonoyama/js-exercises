...
describe("check getDaysExcludeWeekend() function", () => {
  ...
  test.each(testArr2)(
    "getDaysExcludeWeekend(%s, %s) : %s days",
    (date1, date2, result) => {
      expect(getDaysExcludeWeekend(date1, date2)).toBe(result);
    },
  );
  test.each(testArr2Error)(
    "getDaysExcludeWeekend(%s, %s) : %s days",
    (date1, date2, error) => {
...
describe("check getDayOfWeek() function", () => {
  ...
  test.each(testArr3)(
    "getDayOfWeek(%s, %s) : %s days",
    (date, locale, result) => {
      expect(getDayOfWeek(date, locale)).toBe(result);
    },
  );
  test.each(testArr3Error)(
    "getDayOfWeek(%s, %s) : %s",
    (date, locale, error) => {
...
describe("check getFirstDayOfLastMonth() function", () => {
  ...
  expect(getFirstDayOfLastMonth()).toEqual(
    new Date(now.getFullYear(), now.getMonth() - 2, 1, 0, 0, 0),
  );
});
