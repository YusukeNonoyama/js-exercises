...
describe('Has31Days with Elseif', () => {
    test.each(test_array_normal)("Normal: %s => %s", (input, expected) => {
        expect(JSON.stringify(Has31Days.withElseif(input))).toBe(JSON.stringify(expected));
    });
...
describe('Has31Days with Switch', () => {
    test.each(test_array_normal)("Normal: %s => %s", (input, expected) => {
        expect(JSON.stringify(Has31Days.withSwitch(input))).toBe(JSON.stringify(expected));
    });
    test.each(test_array_error)("Error: %s => %s", (input, expected) => {
...
describe('Has31Days with Else if ver2', () => {
    test.each(test_array_normal)("Normal: %s => %s", (input, expected) => {
        expect(JSON.stringify(Has31Days.withElseif2(input))).toBe(JSON.stringify(expected));
    });
    test.each(test_array_error)("Error: %s => %s", (input, expected) => {
